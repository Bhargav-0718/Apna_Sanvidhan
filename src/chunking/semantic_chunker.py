...
class SemanticChunker:
    ...
    def __init__(
        ...
        Args:
            ...
            cache: Optional EmbeddingCache for caching embeddings
    ...
    def compute_sentence_similarities(self, sentences: List[str]) -> List[float]:
        ...
        # Step 1: Embed sentences using BATCH PROCESSING (faster with caching)
        logger.info(f"Computing embeddings for {len(sentences)} sentences using batch processing")
        embeddings = self.batch_processor.batch_embed_texts(
            sentences,
            cache=self.cache,
