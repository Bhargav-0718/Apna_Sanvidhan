"""Main SemRAG pipeline for Apna Sanvidhan - Constitution of India.
...
class ApnaSanvidhan:
    """Main SemRAG pipeline for Apna Sanvidhan - Constitution of India system."""
    
    def __init__(self, config_path: str = "config.yaml"):
        """Initialize Apna Sanvidhan pipeline.
        ...
        logger.info("Initializing Apna Sanvidhan - Constitution of India pipeline")
    ...
    def _setup_logging(self):
        ...
        log_file = log_config.get("file", "apna_sanvidhan.log")
    ...
    def process_document(self, pdf_path: Optional[str] = None, text: Optional[str] = None):
        ...
        if _file_nonempty(data_config["processed_chunks"]):
            ...
            self.chunker = SemanticChunker(
                ...
                cache=self.cache,
                batch_size=self.config["chunking"].get("batch_size", 10)
            )
        ...
        if _file_nonempty(data_config["entities"]):
            ...
            logger.info("Step 3: Entity Extraction (with parallel processing)")
            batch_size = self.config["entity_extraction"].get("batch_size", 5)
            max_workers = self.config["entity_extraction"].get("max_workers", 5)
            use_extraction_cache = self.config["entity_extraction"].get("use_extraction_cache", True)
            skip_duplicates = self.config["entity_extraction"].get("skip_duplicate_chunks", True)
            ...
            self.entities, self.relationships = self.entity_extractor.extract_from_chunks_batch(
                self.chunks,
